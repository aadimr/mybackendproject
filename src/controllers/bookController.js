...
const getBooksInYear = async function (req, res) {
    let yearlyBook = await BookModel.find({ year: 2017 })
    res.send({ msg: yearlyBook })
}
...
const getParticularBooks = async function (req, res) {
    ...
    let year = Data.year
    const Books = await BookModel.find({ bookName: bookName })
    const years = await BookModel.find({ year: year })
    if (bookName) {
        res.send({ msg: Books })
    } else if (year) {
        res.send({ msg: years })
...
//     let allXINRBooks = await BookModel.find({$or:{{prices:{indianPrice:"Rs.115"}},{prices:{indianPrice:"Rs.385"}},{prices:{indianPrice:"Rs.585"}}}})

//     res.send({ msg: allXINRBooks })
// }

const getRandomBooks = async function (req, res){
    const Pages = await BookModel.find({$or:[{totalPages:{$gt:300}},{stockAvailable:{$eq:true}}]})
    res.send({ msg: Pages})
